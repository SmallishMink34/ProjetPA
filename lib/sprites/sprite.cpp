...
void Sprite::SetImage(std::string lien, int x, int y, int w, int h) {
  this->lien = lien;
  this->x = x;
  this->y = y;
  this->w = w;
...
void Sprite::selfDraw(SDL_Renderer* Renderer, int x, int y) {
  ...
  SDL_RenderCopyEx(Renderer, this->GetImg(), &srcRect, &rect, 0, NULL, SDL_FLIP_NONE);
}
